...
static void send_100_packet(uint16_t dest_addr)
{
    ...
    static char pluspack[95]="6789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789\0";
...
static void sync(portTickType* t_start)
{
    // Sync time series
    uint8_t i;
...
{   
    ...
    for(i = 0; i<16; i++)
    {
        ...
        for(j = 0; j<MEASUREMENTS_PER_CASE;)
        {
            if(((phy_rf2xx_t*) platform_phy)->state != PHY_STATE_IDLE)
            {
                log_error("TRANSMISSION SKIPPED. Wrong state %s", state_to_string(((phy_rf2xx_t*) platform_phy)->state));
                continue;
            }
            j++;
            /**
             * Unicast MAC adresses - Site: Grenoble
             * M3-2: 9982, M3-3: b868, M3-100: b080, M3-101: 0x9181, M3-102: 0xa881
             */
            printf("radio > TX - %u\n", j);
