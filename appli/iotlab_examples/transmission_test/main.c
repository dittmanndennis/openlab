...
int main()
{
    ...
    xTaskCreate(app_task, (const signed char * const) "transmission_test_app_task", configMINIMAL_STACK_SIZE, NULL, 1, NULL);
...
static void app_task(void *param)
{
    ...
    //vTaskDelay(configTICK_RATE_HZ * 60 * node_num);
    ...
    for(i = 0; i<16; i++)
    {
        ...
        // Delay send_packet() by 1 second
        vTaskDelay(configTICK_RATE_HZ);
        broadcast_100_packet(tx_power_list[i]);

        // 1 second delay
        vTaskDelay(configTICK_RATE_HZ);
        if(iotlab_uid() == 0x9181)
        {
            unicast_100_packet(tx_power_list[i], 0xb080);
        }
        else
        {
            unicast_100_packet(tx_power_list[i], 0x9181);
        }

        // Unicast
        /*for(uint16_t j = 0; j<1307; j++)
        {
            if((0xf & ((&nodes_uid_dict[j])->node >> 24)) == M3)
            {
                send_25_packet();
            }
        }*/
