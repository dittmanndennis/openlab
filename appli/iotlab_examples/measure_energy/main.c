...
int main()
{
    ...
    xTaskCreate(app_task, (const signed char * const) "measure_energy_app_task", configMINIMAL_STACK_SIZE, NULL, 1, NULL);
...
static void app_task(void *param)
{   
    vTaskDelay(configTICK_RATE_HZ * 9);
    ...
    uint8_t i;
    portTickType t_end = xTaskGetTickCount() + (CASE_DURATION - 1) * configTICK_RATE_HZ;
    for(i = 0; i<9; i++)
    {
        vTaskDelayUntil(&t_end, CASE_DURATION * configTICK_RATE_HZ);
        phy_reset(platform_phy);
        rf2xx_wakeup(((phy_rf2xx_t*) platform_phy)->radio);
        //phy_idle(platform_phy);
        if(sync & 0x80)
        {
            printf("> Jam\n\n");
            t_end = xTaskGetTickCount();
            phy_jam(platform_phy, CHANNEL, PHY_POWER_0dBm);
        }
        else
        {
            printf("> Sleep\n\n");
    ...
    /**
     * PLL_ON
     * -> not measure, because only enabled for BUSY_TX
     *
    vTaskDelayUntil(&t_end, CASE_DURATION * configTICK_RATE_HZ);
    phy_idle(platform_phy);
    t_end = xTaskGetTickCount();
    ...
    */
    ...
    for(i = 0; i<16;)
    {
        ...
        t_end = xTaskGetTickCount();
        send_100_packet(tx_power_list[i], 0x9982);
        ...
        t_end = xTaskGetTickCount();
        send_100_packet(tx_power_list[i], ADDR_BROADCAST);
    ...
    if(((phy_rf2xx_t*) platform_phy)->state != PHY_STATE_IDLE)
    {
        log_error("Wrong state %s", state_to_string(((phy_rf2xx_t*) platform_phy)->state));
    }
    ...
    // 20 min delay
    vTaskDelayUntil(&t_end, 240 * CASE_DURATION * configTICK_RATE_HZ);
}
